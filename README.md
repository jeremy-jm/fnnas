...
| s805 | WS1508 (ZhuanQianBao2, 512MB RAM/4GB storage, 100Mb LAN, 1xUSB2, RGB LED), WS1608 (ZhuanQianBao3, 1GB RAM/8GB storage, 1Gb LAN, 2xUSB2, SD, HDMI, RGB LED) | [fnnas](https://github.com/ophub/fnnas/releases/tag/kernel_fnnas) |
...
1. Enter the `~/fnnas` root directory, create a folder named `fnnas-arm64` (or `fnnas-armhf` for 32-bit base image) in the root directory, and upload the FnNAS image file (e.g., `fnos_arm_1.0.0_258.img`) to the corresponding directory.

2. Enter the `~/fnnas` root directory, then run the sudo command to generate the FnNAS image file for the specified board. The generated files are saved in the `~/fnnas/out` directory.

Example (armhf base image, keep kernel from base image): `sudo ./renas -p fnnas-armhf -x true -b ws1508`
