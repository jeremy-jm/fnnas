...
| s805 | 赚钱宝2 WS1508（512MB 内存/4G 存储、百兆网口、1×USB2.0、红绿蓝三色指示灯），赚钱宝3 WS1608（1G 内存/8G 存储、千兆网口、2×USB2.0、SD 读卡器、HDMI、红绿蓝三色指示灯） | [fnnas](https://github.com/ophub/fnnas/releases/tag/kernel_fnnas) |
...
3. 进入 `~/fnnas` 根目录，在根目录下创建文件夹 `fnnas-arm64`（32 位基础镜像使用 `fnnas-armhf`），并上传 FnNAS 镜像文件（如：`fnos_arm_1.0.0_258.img`）到对应目录里。

4. 进入 `~/fnnas` 根目录，然后运行命令即可生成指定 board 的 FnNAS 镜像文件。生成的文件保存在 `~/fnnas/out` 目录里。

示例（armhf 基础镜像，保留基础镜像自带内核）：`sudo ./renas -p fnnas-armhf -x true -b ws1508`
